...
class MinMaxStack {
    ...
    push(val) {
        ...
        if (!this.minStorage.length || val <= this.minStorage[this.minStorage.length - 1].value) {
            this.minStorage.push(newNode);
        }

        if (!this.maxStorage.length || val >= this.maxStorage[this.maxStorage.length - 1].value) {
